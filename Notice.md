...
-	The optee_linuxdriver git, containing the source code for the TEE driver in Linux. This component implements a generic TEE driver, designed primarily for TEE implementations that rely on the ARM TrustZone™ technology. It is distributed under the GPLv2 open-source license. Please note that re-distribution under other versions of the GPL license is not allowed. The rationale for this limitation is that the BSD license has been chosen for the TEE OS in order to allow performing modifications to the TEE OS without disclosing them. Allowing distribution of any other related code under a license such as GPLv3 would therefore be inconsistent with this goal.
Contributions
------
