...
copy_export: build
	...
	cp ${shell dirname $(EXPORT_DIR)}/libteec/libteec.so* ${EXPORT_DIR}/lib
	if [ "$(BUILD-LIBSQLFS)" ]; then cp ${shell dirname $(EXPORT_DIR)}/libsqlfs/libsqlfs.so* ${EXPORT_DIR}/lib; fi
	cp ${shell dirname $(EXPORT_DIR)}/tee-supplicant/tee-supplicant ${EXPORT_DIR}/bin
